...
# Function to get Entrez IDs for gene symbols using org.Hs.eg.db
get_entrez_ids <- function(gene_symbols) {
  # Install/load required packages for gene ID conversion
  ...
  # Remove NAs and create a data frame
  valid_genes <- !is.na(entrez_ids)
  gene_info <- data.frame(
    hugoGeneSymbol = gene_symbols[valid_genes],
    entrezGeneId = as.numeric(entrez_ids[valid_genes]),
    stringsAsFactors = FALSE
  )
  
  cat("Found Entrez IDs for", nrow(gene_info), "out of", length(gene_symbols), "gene symbols\n")
  
  # Show which genes were not found
  not_found <- gene_symbols[!valid_genes]
  if (length(not_found) > 0) {
    cat("Genes not found:", paste(not_found, collapse = ", "), "\n")
  }
  
  return(gene_info)
}

# Function to query mutations for ARHG genes
query_arhg_mutations <- function(study_ids, gene_info) {
  results <- list()
  ...
  for (study_id in study_ids) {
    ...
    try({
      ...
      if (nrow(mut_profile) > 0) {
        # Try different sample list formats
        sample_lists <- c(
          paste0(study_id, "_all"),
          paste0(study_id, "_sequenced"),
          paste0(study_id, "_cnaseq")
        )
        
        mutations <- NULL
        for (sample_list in sample_lists) {
          try({
            # Query mutations using Entrez IDs
            mutations <- mutationData(
              cbio,
              molecularProfileIds = mut_profile$molecularProfileId[1],
              entrezGeneIds = gene_info$entrezGeneId,
              sampleListId = sample_list
            )
            
            if (nrow(mutations) > 0) {
              cat("  Found", nrow(mutations), "mutations using sample list:", sample_list, "\n")
              break
            }
          }, silent = TRUE)
        }
