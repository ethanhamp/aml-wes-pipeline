...
my_studies <- studies[studies$studyId %in% selected_ids, ]
write.csv(studies, file.path(BASE_DIR, "cBioPortal/Datasets/cBioPortal_studies_list.csv"), row.names = FALSE)

all_genes <- get_genes(cbio)
genes <- read.delim(file.path(BASE_DIR, "General/hgnc_complete_set.txt"), header = TRUE, sep = "\t")

arhg_genes <- read.csv(file.path(BASE_DIR, "General/arhg_genes.csv"), header = TRUE)
arhg_genes1 <- arhg_genes$symbol
arhg_genes2 <- c(
  # ARHGEF genes (Rho guanine nucleotide exchange factors)
  ...
  # ARHGAP genes (Rho GTPase activating proteins)
...
#   message ("Downloading mutations for ", study_id)
#   study_data <- cBioDataPack(study_id, use_cache=TRUE)
#   mut <- assay(study_data, "mutations")
#   arhg_mut <- mut[mut$Hugo_Symbol %in% arhg_genes, ]
#   results[[study_id]] <- arhg_mut
# }

query_arhg_mutations <- function(study_ids, genes) {
  results <- list()
  
  for (study_id in study_ids) {
    cat("Querying study:", study_id, "\n")
    
    try({
      # Get molecular profiles for the study
      mol_profiles <- molecularProfiles(cbio, studyId = study_id)
      
      # Find mutation profile
      mut_profile <- mol_profiles[grepl("mutations", mol_profiles$molecularAlterationType), ]
      
      if (nrow(mut_profile) > 0) {
        # Query mutations
        mutations <- mutationData(
          cbio,
          molecularProfileIds = mut_profile$molecularProfileId[1],
          entrezGeneIds = genes,
          sampleListId = paste0(study_id, "_all")
        )
        
        if (nrow(mutations) > 0) {
          mutations$studyId <- study_id
          results[[study_id]] <- mutations
        }
      }
    }, silent = TRUE)
  }
...
# Query mutations
cat("Querying mutations for ARHG genes...\n")
mutation_results <- query_arhg_mutations(selected_studies, arhg_genes2)

# Combine results
if (length(mutation_results) > 0) {
  all_mutations <- do.call(rbind, mutation_results)
  cat("Found", nrow(all_mutations), "mutations across", length(mutation_results), "studies\n")
  
  # Summary by gene
  mutation_summary <- all_mutations %>%
    group_by(hugoGeneSymbol, studyId) %>%
    summarise(
      n_mutations = n(),
  ...
  cat("No mutations found for the selected studies and genes\n")
}
