...
map_token_to_maf <- function(tok, variant_type) {
  ...
  dplyr::case_when(
    ...
    t %in% c("stopgain","stop_gained","stop_gained_variant") ~ "Nonsense_Mutation",
    t %in% c("stoploss","stop_lost","stop_lost_variant","nonstop_mutation") ~ "Nonstop_Mutation",
    t %in% c("startloss","start_lost","initiator_codon","initiator_codon_variant") ~ "Translation_Start_Site",
    ...
    str_detect(t, "nonframeshift insertion|inframe_insertion|dup") ~ "In_Frame_Ins",
    
    # missense / synonymous
    t %in% c("missense","missense_variant","nonsynonymous snv","nonsynonymous_snv") ~ "Missense_Mutation",
...
diagnosis_df <- DF[!is.na(DF$Timepoint) & DF$Timepoint == "diagnosis" & DF$`Alt Percentage`>=0.02, , drop = FALSE]
relapse_df <- DF[!is.na(DF$Timepoint) & DF$Timepoint == "relapse" & DF$`Alt Percentage`>=0.02, , drop = FALSE]
...
relapse_maf <- read.maf(maf = relapse_df)
...
oncoplot(maf = diagnosis_maf, 
         top = 30, 
         legendFontSize = 1.4,
         fontSize = 0.7,
         titleText = "Diagnosis samples (n=29)")
...
diag.maf.sig = oncodrive(maf = maf, AACol = 'MANE HGVS', minMut = 5, pvalMethod = 'zscore')
...
oncoplot(maf = relapse_maf, 
         top = 30, 
         legendFontSize = 1.4,
         fontSize = 0.7,
         titleText = "Relapse samples (n=29)")
plotVaf(maf = maf, vafCol = 'Alt Percentage')
...
genes = c("NPM1", "FLT3", "TET2", "ARHG", "NRAS", "PHIP", "DNMT3A", "ASXL1", "HOMEZ", "IDH2",
          "SRSF2", "WT1", "TP53", "TUBB8B", "DEAF1", "SLC", "ZNF", "PCDH", "DOCK")
coOncoplot(m1 = diagnosis_maf, 
           m2 = relapse_maf, 
           m1Name = 'Diagnosis AML', 
           m2Name = 'Relapse AML', 
           genes = genes,
           removeNonMutated = FALSE)

coBarplot(m1 = diagnosis_maf, 
          m2 = relapse_maf, 
          m1Name = "Diagnosis", 
          m2Name = "Relapse", 
          genes = genes,
